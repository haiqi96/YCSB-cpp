...
namespace {
  ...
  const std::string PROP_DESTROY_DEFAULT = "true";
...
// this only deserialize if value also appears in the given input fields
// however similar logic isn't seen in the yscb java
...
DB::Status RocksdbDB::ScanSingle(const std::string &table, const std::string &key, int len,
  ...
  //std::cout << "scan single with length" << len << std::endl;
...
DB::Status RocksdbDB::UpdateSingle(const std::string &table, const std::string &key,
  ...
  std::vector<Field> result;
  DB::Status s = ReadSingle(table, key, &fields, result);
  ...
  s = InsertSingle(table, key, values);
  if (s != kOK) {
...
DB::Status RocksdbDB::DeleteSingle(const std::string &table, const std::string &key) {
  std::cout << "delete single" << std::endl;
  rocksdb::WriteOptions wopt;
